  ...
  const uploadImage = () => {
    if (image === null) alert("please select an image first");
      ...
      uploadBytes(imageRef, image)
        .then((snapshots) => {
          getDownloadURL(snapshots.ref).then((url) => setimgurls(url));
          setMicstate(true);
        })
  ...
  return (
    <div className="  text-[#000000] h-full flex flex-col items-center justify-start">
      <div className=" flex flex-col items-center justify-start pt-5">
        <label
          htmlFor="Uploader"
          className="border-4  border-[#5120f2bc] border-dashed h-64 whitespace-nowrap flex flex-col items-center justify-center w-[25vw] p-8"
        >
          {imgUrls ? (
            <img
              src={imgUrls}
        ...
        <div className="flex  gap-8 w-full px-3 justify-center">
          <button
            type="button"
            disabled={uploderState === "disabled" ? "disabled" : ""}
            className={
              image
                ? uploderState === "disabled"
                  ? "bg-[#26106e6f]  w-full max-w-44 h-12 mt-9 rounded-lg"
                  : "bg-[#5120f2bc] hover:bg-[#5120f2] w-full max-w-44 h-12 mt-9 rounded-lg"
                : "hidden"
            }
            onClick={uploadImage}
          >
          ...
          <button
            ...
            className={
              image
                ? "bg-[#f7e5e5] hover:bg-[tomato]  w-full max-w-44 h-12 mt-9 rounded-lg"
                : "hidden"
            }
            onClick={removeImage}
          >
